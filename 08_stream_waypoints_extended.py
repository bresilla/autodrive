...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"maximum ADWPI frames per batch, including overlap; 0 means one burst "
...
def stream_all_waypoints(bus, status, waypoints, batch_size: int):
    ...
    if batch_size <= a.WINDOW_OVERLAP_POINTS and batch_size < len(waypoints):
        raise ValueError(f"--batch-size must be 0 or greater than overlap "
                         f"({a.WINDOW_OVERLAP_POINTS})")
    current_index = 0
    batches = []
    total_frames = 0
    unique_sent: set[int] = set()
    while current_index < len(waypoints):
        ...
        total_frames += sent
        unique_sent.update(wp.index for wp in waypoints[start:end])
        if end >= len(waypoints):
            break
        current_index = end
    return batches, total_frames, unique_sent
...
def main() -> None:
    ...
    batch_label = "one burst" if args.batch_size == 0 else str(args.batch_size)
    ...
    batches, total_frames, unique_sent = stream_all_waypoints(bus, status, waypoints, args.batch_size)

    for i, (start, end, sent) in enumerate(batches, start=1):
        unique_start = 0 if i == 1 else start + a.WINDOW_OVERLAP_POINTS
        print(f"batch {i}: streamed indices [{start}..{end - 1}], {sent} frames "
              f"(new [{unique_start}..{end - 1}])")
    missing = [i for i in range(len(waypoints)) if i not in unique_sent]
    if missing:
        raise SystemExit(f"internal streaming error: missing waypoint indices {missing[:20]}")
    print(f"\nstreamed every unique waypoint index 0..{len(waypoints) - 1} "
          f"({len(unique_sent)} points) in {len(batches)} batches, "

...
Default mode sends an anchor-only ADJOB SystemActive=true first, then waits for DSAP:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="ADJOB total point count (default: 0 for anchor-only request)",
    )
    parser.add_argument(
        "--route",
    ...
    parser.add_argument(
        "--no-inside-gate",
        action="store_true",
        help="do not require the machine position to be inside the route field (default)",
    )
    parser.add_argument(
        "--inside-gate",
        action="store_true",
        help="require the machine position to be inside the route field",
    )
...
def ready_to_activate(args: argparse.Namespace, status: a.MachineStatus,
    ...
    if not status.gps_ppp_available or not status.autodrive_allowed:
        return False
    if args.inside_gate and not inside_field(status, field, datum_lat, datum_lon):
        return False
    return True
...
def main() -> None:
    ...
    if not args.passive:
        print(f"route={args.route!r} total_points={total_points} inside_gate={'on' if args.inside_gate else 'off'}")
    ...
    while deadline is None or time.monotonic() < deadline:
        ...
        if not args.passive and now - last_request >= a.ADJOB_PERIOD_S:
            ...
            print(f"TX ADJOB systemActive={yn(active)} run=- job_id={job_id} "
                  ...
                  f"inside={yn(inside) if args.inside_gate else 'skip'} "
        ...
        if status.anchor_lat is None or status.anchor_lon is None:
            ...
            continue
        if not args.passive and not active_sent:
            print("valid DSAP received before this script sent SystemActive=Y; ignoring old/current job anchor")
            continue

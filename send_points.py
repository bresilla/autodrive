...
This keeps 08_stream_waypoints.py untouched. The extended script is a live loop:
  ...
  * send the first N unique route points, default 200
...
MAX_PROGRESS_JUMP = 5
DEFAULT_NEAREST_AHEAD = MAX_PROGRESS_JUMP
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"route points ahead to search for GPS progress, capped at {MAX_PROGRESS_JUMP} "
             f"to prevent crossing jumps (default: {DEFAULT_NEAREST_AHEAD})",
    )
    parser.add_argument(
        "--nearest-backtrack",
        type=int,
...
def estimate_progress_index(
    ...
    """Monotonic GPS progress, limited to nearby forward route segments."""
    ...
    # Do not let a geometrically close segment from a later crossing win. Searching
    # segments previous..previous+N-1 permits an advance of at most N points.
    forward_search = min(nearest_ahead, MAX_PROGRESS_JUMP)
    last_seg = min(len(xy) - 2, previous + forward_search - 1)
